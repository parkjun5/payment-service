...
class R2DBCPaymentRepository (
    ...
    private val transactionalOperators: TransactionalOperator
) : PaymentRepository {
    ...
    override fun save(paymentEvent: PaymentEvent): Mono<Unit> {
        return insertPaymentEvent(paymentEvent)
            ...
            .`as` { transactionalOperators.transactional(it) }
            .thenReturn(Unit)
    }
    ...
    private fun selectPaymentEventId() = databaseClient.sql(LAST_INSERT_TO_QUERY)
        .fetch()
        .first()
